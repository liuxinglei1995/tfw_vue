  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="新价格" prop="newPrice">
        <el-input
          v-model="queryParams.newPrice"
          placeholder="请输入新价格"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="调价时间" prop="adjustmentDate">
    ...
    <el-table v-loading="loading" :data="priceList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="小区id" align="center" prop="houseId" />
      ...
      <el-table-column label="调价时间" align="center" prop="adjustmentDate" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.adjustmentDate, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="小区id" prop="houseId">
          <el-input v-model="form.houseId" placeholder="请输入小区id" />
        </el-form-item>
        <el-form-item label="旧价格" prop="oldPrice">
          <el-input v-model="form.oldPrice" placeholder="请输入旧价格" />
        </el-form-item>
        <el-form-item label="新价格" prop="newPrice">
          <el-input v-model="form.newPrice" placeholder="请输入新价格" />
        </el-form-item>
        <el-form-item label="调价时间" prop="adjustmentDate">
          <el-date-picker clearable
...
<script>
import { listPrice, getPrice, delPrice, addPrice, updatePrice } from "@/api/price/price";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除楼盘均价调价记录编号为"' + ids + '"的数据项？').then(function() {
        return delPrice(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('price/price/export', {
        ...this.queryParams
