<template>
  <div class="app-container">
    ...
    >
      ...
      >
        <el-input
          v-model="queryParams.idPhoneName"
          placeholder="请输入信息"
          clearable
          style="width: 170px"
        />
      ...
      <el-row>
        ...
        >
          <el-select
            v-model="queryParams.ifNewHouse"
            clearable
            placeholder="请选择"
            style="width: 120px"
          >
            <el-option
              v-for="dict in dict.type.tfw_new_old"
              :key="dict.value"
              :label="dict.label"
              :value="dict.value"
            ></el-option>
        ...
        >
          ...
          >
            <el-option
              v-for="dict in dict.type.tfw_state_2"
              :key="dict.value"
              :label="dict.label"
              :value="dict.value"
            ></el-option>
        ...
        <el-form-item label="装修" prop="state" size="mini" label-width="80px">
          <el-select
            v-model="queryParams.decorationIds"
            multiple
            placeholder="请选择装修情况"
          >
            <el-option
              v-for="dict in dict.type.tfw_decoration_situation"
              :key="dict.value"
              :label="dict.label"
        ...
        <el-form-item prop="lift" style="margin-bottom: 0px" label="视频:">
          <el-radio-group v-model="queryParams.params.haveVideo" size="mini">
            <el-radio-button
              v-for="dict in isVideo"
              :key="dict.value"
              :label="dict.value"
              size="medium"
              @click.native="pickLift($event)"
              >{{ dict.label }}
            </el-radio-button>
          </el-radio-group>
        </el-form-item>
        ...
        <div style="display: flex; align-items: center; margin-right: 10px">
          ...
          <div class="content">
            <el-form-item prop="lift" style="margin-bottom: 0px">
              <el-radio-group v-model="queryParams.params.lift" size="mini">
                <el-radio-button
                  v-for="dict in dict.type.tfw_elevator_situation"
                  :key="dict.value"
                  :label="dict.value"
                  size="medium"
                  @click.native="pickLift($event)"
          ...
          <div class="content">
            <el-form-item prop="huxingId" style="margin-bottom: 0px">
              <el-radio-group v-model="queryParams.huxingId" size="mini">
                <el-radio-button
                  v-for="dict in dict.type.tfw_fuxing"
                  :key="dict.value"
                  :label="dict.value"
                  size="medium"
                  @click.native="pickHuxing($event)"
                  >{{ dict.label }}
                </el-radio-button>
              </el-radio-group>
            </el-form-item>
      ...
      <div class="row">
        ...
        <div class="content">
          <el-form-item prop="params.floor" style="margin-bottom: 0px">
            <el-input
              v-model="queryParams.params.floorMin"
              placeholder="请输入最小值"
              clearable
              style="width: 120px"
              size="mini"
            ...
            <el-input
              v-model="queryParams.params.floorMax"
              placeholder="请输入最大值"
              clearable
            ...
            <el-radio-group v-model="queryParams.params.floor" size="mini">
              <el-radio-button label="1" @click.native="pickFloor($event)"
                >楼层从低到高</el-radio-button
              >
              <el-radio-button label="2" @click.native="pickFloor($event)"
                >楼层从高到低</el-radio-button
              >
            </el-radio-group>
          </el-form-item>
        </div>
        ...
        <div class="content">
          <el-form-item prop="propertyArea" style="margin-bottom: 0px">
            <el-radio-group v-model="queryParams.propertyArea" size="mini">
              <el-radio-button
                v-for="dict in dict.type.tfw_arear"
                :key="dict.value"
                :label="dict.value"
                size="medium"
                @click.native="pickArea($event)"
                >{{ dict.label }}
              </el-radio-button>
            </el-radio-group>
            <el-input
              v-model="queryParams.params.propertyAreaMin"
              placeholder="请输入最小值"
              clearable
            ...
            <i class="el-icon-minus"></i>
            <el-input
              v-model="queryParams.params.propertyAreaMax"
              placeholder="请输入最大值"
              clearable
      ...
      <div class="row" v-if="queryParams.type == 1 || queryParams.type == 3">
        ...
        <div class="content">
          <el-form-item prop="sellingPrice" style="margin-bottom: 0px">
            <el-radio-group v-model="queryParams.sellingPrice" size="mini">
              <el-radio-button
                v-for="dict in dict.type.tfw_prices"
                :key="dict.value"
                :label="dict.value"
                size="medium"
                @click.native="pickPrice($event)"
                >{{ dict.label }}
              </el-radio-button>
            </el-radio-group>
            <el-input
              v-model="queryParams.params.sellingPriceMin"
              placeholder="请输入最小值"
              clearable
            ...
            <i class="el-icon-minus"></i>
            <el-input
              v-model="queryParams.params.sellingPriceMax"
              placeholder="请输入最大值"
              clearable
              style="width: 120px; margin-right: 20px"
              size="mini"
      ...
      <div class="row" v-if="queryParams.type == 2 || queryParams.type == 3">
        <div class="name">租金:</div>
        <div class="content">
          <el-form-item prop="rentPrice" style="margin-bottom: 0px">
            <el-input
              v-model="queryParams.params.rentPriceMin"
              placeholder="请输入最小值"
              clearable
              style="width: 120px; margin-left: 20px"
              size="mini"
            ...
            <i class="el-icon-minus"></i>
            <el-input
              v-model="queryParams.params.rentPriceMax"
              placeholder="请输入最大值"
              clearable
              style="width: 120px; margin-right: 20px"
              size="mini"
            />
            <el-radio
              v-model="queryParams.params.rentSort"
              label="1"
              border
              @click.native="pickSort($event)"
              >租金从低到高</el-radio
        ...
        <div class="content">
          <el-form-item prop="propertyType" style="margin-bottom: 0px">
            <el-radio-group v-model="queryParams.propertyType" size="mini">
              <el-radio
                v-for="dict in dict.type.tfw_property_type"
                :key="dict.value"
                :label="dict.value"
                size="medium"
                @click.native="pickProperty($event)"
                >{{ dict.label }}
              </el-radio>
            </el-radio-group>
          </el-form-item>
      ...
      <div class="container" v-show="showSearch">
        <div class="CationBox">
          ...
          >
            ...
            >
              <el-option
                v-for="dict in dict.type.tfw_ecommendation_level"
                :key="dict.value"
                :label="dict.label"
                :value="dict.value"
              ></el-option>
          ...
          <el-form-item
            label="房屋来源"
            prop="entrustmentMethod"
            size="mini"
            label-width="80px"
          >
            <el-select
              v-model="queryParams.entrustmentMethod"
              placeholder="请选择情况"
              style="width: 120px"
            >
              <el-option
                v-for="dict in dict.type.tfw_source_commission"
                :key="dict.value"
                :label="dict.label"
                :value="dict.value"
              ></el-option>
          ...
          <el-form-item label="维护人" prop="createByName" size="mini">
            <el-input
          ...
          <el-form-item label="所属门店" prop="deptId" label-width="80px">
            <treeselect
          ...
          >
            <el-select
              v-model="queryParams.ownershipYears"
              placeholder="请选择产权年限"
              style="width: 130px"
            >
              <el-option
                v-for="dict in dict.type.tfw_property_rights"
                :key="dict.value"
                :label="dict.label"
                :value="dict.value"
              ></el-option>
          ...
          >
            ...
            >
              <el-option
                v-for="dict in dict.type.tfw_status"
                :key="dict.value"
                :label="dict.label"
                :value="dict.value"
              ></el-option>
          ...
          >
            <el-select
              v-model="queryParams.kfang"
              placeholder="请选择情况"
              style="width: 120px"
            >
              <el-option
                v-for="dict in dict.type.tfw_kffs"
                :key="dict.value"
                :label="dict.label"
                :value="dict.value"
              ></el-option>
    ...
    >
          ...
          <div style="display: flex; align-items: center">
            ...
            >
              ...
              <div style="display: flex; flex-wrap: wrap">
                <div
                  :class="[
                    scope.row.type == 1 ? 'one' : 'two',
                    scope.row.type == 3 ? 'three' : '',
                  ]"
                >
                  <dict-tag
                    :options="dict.type.tfw_rent_sale"
                ...
                <div
                  class="jbxx"
                  :class="[scope.row.status == 2 ? 'four' : 'five']"
                >
                  <dict-tag
                    :options="dict.type.tfw_status"
                ...
                <div
                  class="jbxx"
                  v-else
                  style="background-color: #ffeded; color: #ff9292"
                >
                  <div>未上传产权证</div>
                </div>
              </div>
      ...
      <el-table-column label="户型" align="center" prop="huXTitle">
        <template slot-scope="scope">
          <div>
            {{
              scope.row.loupanHuxing ? scope.row.loupanHuxing.bedRoomNum : "--"
            }}室{{
        ...
        <template slot-scope="scope">
          <div style="display: flex; flex-direction: column">
            ...
            <el-button
              ...
              @click.stop="looksellingPrice(scope.row.id)"
              >调价记录
            </el-button>
          </div>
        </template>
      ...
      >
        <template slot-scope="scope">
          ...
          <div v-else>* - * - * - *</div>
          <div style="color: #ff7200; margin-bottom: 5px">
            {{
              scope.row.houseFollow
                ? getDiffDay(scope.row.houseFollow.createdAt) + "天未跟进"
                : "暂无跟进"
            }}
          </div>
        </template>
        ...
        <template slot-scope="scope">
          <div v-if="scope.row.tfwHousePrice">
            <div class="d1" v-if="scope.row.tfwHousePrice.type == 1">
              <sapn class="d2">涨价</sapn
              >{{
                scope.row.tfwHousePrice ? scope.row.tfwHousePrice.reason : "--"
              }}万
            </div>
            <div class="d3" v-if="scope.row.tfwHousePrice.type == 2">
              <sapn class="d4">降价</sapn
              >{{
                scope.row.tfwHousePrice ? scope.row.tfwHousePrice.reason : "--"
              }}万
            </div>
            ...
            <span>
              {{
                scope.row.sellingPrice && scope.row.propertyArea
                  ? Math.round(
                      (scope.row.sellingPrice * 10000) / scope.row.propertyArea
                    )
                  : "-"
              }}元/㎡
            </span>
          ...
          <div
            style="color: #ff7200; margin-bottom: 5px"
            v-if="scope.row.state === '4' || scope.row.state === '6'"
          >
            {{ initRent(scope.row.tfwHouseRentList) }}
          </div>
        </template>
      </el-table-column>
      <el-table-column label="维护人" align="center" prop="nickName">
        <template slot-scope="scope">
          ...
          <div v-if="scope.row.status == 2">
            已上架:
            {{
              scope.row.groundingTime
                ? getDiffDay(scope.row.groundingTime)
                : "--"
            }}天
          </div>
        </template>
      ...
      >
        <template slot-scope="scope">
          <dict-tag
            :options="dict.type.tfw_decoration_situation"
            :value="scope.row.decoration"
          />
        </template>
      </el-table-column>
      <el-table-column
      ...
      >
        <template slot-scope="scope">
          ...
          <el-button
            ...
            @click.stop="handlegenjin(scope.row)"
            v-hasPermi="['system:follow:query']"
            >跟进
          </el-button>
          <el-button
            size="mini"
            ...
            icon="el-icon-price-tag"
            v-hasPermi="['system:house:edit']"
            @click.stop="handlerowlist(scope.row)"
            >详情
          </el-button>
          <el-link
            @click.stop.native
            ...
            size="mini"
            icon="el-icon-price-tag"
            :href="'/poster?id=' + scope.row.id + '&type=' + scope.row.type"
            target="_blank"
            type="primary"
            >水牌</el-link
          >
    ...
    >
      ...
      >
        ...
        <el-col :span="24">
          ...
          <br />
          <el-form-item
            v-for="(domain, index) in phoneNumbercopy"
            :label="'业主' + (index + 1)"
            :key="domain.key"
            :required="index === 0"
          >
            <el-form :model="domain" :rules="rules.phoneNumber" ref="domain">
              <el-form-item prop="type">
                ...
                >
                  <el-option
                    v-for="dict in dict.type.tfw_yzgx"
                    :key="dict.value"
                    :label="dict.label"
                    :value="dict.value"
                  ></el-option>
        ...
        <el-form-item label="租售类型" prop="type">
          ...
          >
            <el-option
              v-for="dict in dict.type.tfw_rent_sale"
              :key="dict.value"
              :label="dict.label"
              :value="dict.value"
            ></el-option>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="addnew" :disabled="isShowmessage"
          >确 定</el-button
        >
      ...
      >
        <el-tabs v-model="activeName2" type="card">
          <el-tab-pane label="基本信息" name="first">
            <div class="messagebox">
              <div class="leftmessagebox">
                ...
                <ImagePreviewHouse
                  ...
                  :housephotos="form.housePhotos"
                  :huxing="form.loupanHuxing"
                  :height="110"
                  :width="120"
                />
              </div>
              <div class="rightmessagebox">
                <div>
                  <span class="price" v-if="form.type == 1 || form.type == 3"
                    >{{ form.sellingPrice }}万</span
                  >
                  <span
                    style="
                      color: #888a8e;
                  ...
                  <el-button
                    ...
                    @click="looksellingPrice(form.id)"
                    >查看价格调价记录
                  </el-button>
                </div>
                <p class="xqname" v-if="form.loupanXq">
                  {{ form.xqname }} {{ form.loupanXq.address }}
                </p>
                <p class="adress" style="display: flex">
                  地址:{{ formatAddressValue(form.building) }}栋{{
                    formatAddressValue(form.unit)
                  }}单元{{ formatAddressValue(form.floor) }}层{{
                    formatAddressValue(form.roomNum)
                  }}号
                  <img
                    :src="
                      addressVisible
                        ? require('@/assets/image/closeeyes.png')
                        : require('@/assets/image/openeyes.png')
                    "
                    alt=""
                    style="width: 20px; height: 20px; cursor: pointer"
                    @click="toggleAddressVisible"
                  />
                </p>
                <p class="recommend">
                  房屋状态:
                  <dict-tag
                    :options="dict.type.twf_fwxz"
                    :value="form.statusQuo"
                  />
                </p>
              </div>
            </div>
            <el-row :gutter="20" class="listtitle">
              <div
                ...
                "
              >
                (以下数据双击数据可修改,修改后回车或点击修改选项)
              </div>
              <el-col :span="6">
                <el-card shadow="hover">
                  <p class="hoversize">户型</p>
                  <div v-if="showshishi" @dblclick="shishiClick(form.id)">
                    {{
                      form.loupanHuxing ? form.loupanHuxing.bedRoomNum : "--"
                    }}室{{
                      form.loupanHuxing
                        ? form.loupanHuxing.livingRoomNum
                        : "--"
                    }}厅{{
                      form.loupanHuxing ? form.loupanHuxing.bathroomNum : "--"
                    }}卫
                  </div>
                  <el-select
                    v-model="form2.huXTitle"
                    placeholder="请选择户型图"
                    filterable
                    @click.native="clickhx"
                    ref="input"
                    v-if="closexq"
                    @change="enterclick()"
                  >
            ...
            >
              <el-descriptions-item label="小区名字">{{
                form.xqname
              }}</el-descriptions-item>
              <el-descriptions-item label="开发商" v-if="form.loupanXq">{{
                form.loupanXq.developer
              }}</el-descriptions-item>
              <el-descriptions-item label="物业公司" v-if="form.loupanXq">{{
                form.loupanXq.pmc
              }}</el-descriptions-item>
              ...
              <el-descriptions-item label="物业费" v-if="form.loupanXq"
                >{{ form.loupanXq.serviceCharge }}元</el-descriptions-item
              >
            </el-descriptions>
            <el-descriptions
              title="业主信息"
            ...
            >
              <el-descriptions-item label="业主">
                <div
                  v-for="(item, index) in form.phoneNumber"
                  :key="index"
            ...
            >
              ...
              >
                <dict-tag
                  :options="dict.type.tfw_source_commission"
                  :value="form.entrustmentSource"
                />
              </el-descriptions-item>
              <el-descriptions-item
                label="委托方式"
              ...
              >
                <dict-tag
                  :options="dict.type.tfw_entrustment_method"
                  :value="form.entrustmentMethod"
                />
              </el-descriptions-item>
              <el-descriptions-item label="归属人信息"
                >{{ form.sysUser ? form.sysUser.dept.deptName : "--" }}
                {{ form.sysUser ? form.sysUser.nickName : "--" }}
                {{
                  form.sysUser ? form.sysUser.phonenumber : "--"
                }}</el-descriptions-item
              >
            </el-descriptions>
            <el-descriptions
            ...
            >
              <el-descriptions-item label="套内面积"
                >{{ form.usableArea }}㎡</el-descriptions-item
              >
              <el-descriptions-item label="赠送面积"
                >{{ form.giveAwayArea }}㎡</el-descriptions-item
              >
              ...
              <el-descriptions-item label="楼层"
                >{{ form.floor }}/{{ form.totalFloor }}F</el-descriptions-item
              >
              <el-descriptions-item label="层高">{{
                form.loupanXq ? form.loupanXq.floorHigh : "--"
              }}</el-descriptions-item>
              <el-descriptions-item label="房屋结构">
                <dict-tag
                  :options="dict.type.tfw_house_structure"
                  :value="form.structure"
                />
              </el-descriptions-item>
              <el-descriptions-item label="电梯">
                <dict-tag
                  :options="dict.type.tfw_elevator_situation"
                  :value="form.lift"
                />
              </el-descriptions-item>
              <el-descriptions-item label="学校">{{
                form.loupanXq ? ellipsis(form.loupanXq.schoolDistrict) : "--"
              }}</el-descriptions-item>
              <el-descriptions-item label="产权年限">
                <dict-tag
                  :options="dict.type.tfw_property_rights"
              ...
              <el-descriptions-item label="是否满五">{{
                getfivenear(form.titleDeedDate)
              }}</el-descriptions-item>
            ...
            <el-descriptions
              ...
              style="padding-right: 20px"
            >
              <el-descriptions-item label="原持有人"
                >转移时间</el-descriptions-item
              >
              <el-descriptions-item
                v-for="item in transferList"
                :key="item.id"
                :label="item.nickName"
                >{{ formatDate(item.transferTime) }}</el-descriptions-item
              >
            </el-descriptions>
          </el-tab-pane>
          <!-- 详细介绍 -->
          ...
          <el-tab-pane label="房源相册" name="image">
            <div
              style="padding: 10px 40px; box-sizing: border-box"
              class="protected-area"
            >
              <div class="overlay" v-if="!drawerIfUpdate"></div>
              <el-popover
                placement="right"
                title="扫码上传图片"
                width="200"
              ...
              >
                ...
                <el-button slot="reference" @click="qrcodeclick()"
                  >生成扫码上传图片二维码</el-button
                >
              </el-popover>
              <list-image-upload
                :data="form"
                :limit="10"
                :titleDeeds="form.titleDeeds"
                :dialogImageUrl="form.video"
                :isType="1"
                :superiorId="form.id"
                :huxingimg="form.loupanHuxing"
              />
              <el-divider></el-divider>
              <div style="padding:10px 0;">
                <div style="font-size:14px;font-weight:bold;margin-bottom:8px;">权证信息</div>
                  <el-form :model="form" size="small" label-width="100px" inline>
                    <el-form-item label="权证业务号">
            ...
            >
              ...
              >
                ...
                <el-form-item label="看房类型">
                  <el-select
                    v-model="shelfData.kfang"
                    placeholder="请选择"
                    style="width: 150px"
                    @change="changeSelect(shelfData, 3)"
                  >
                    <el-option
                      v-for="dict in dict.type.tfw_kffs"
                      :key="dict.value"
                ...
                <!-- 电子密码 -->
                <el-form-item v-if="shelfData.kfang == 4" label="电子密码" prop="housePwd">
                  <!-- 保持 flex 布局以容纳输入框和按钮 -->
                  <div style="display: flex; gap: 5px; width: 220px">
                    <el-input
                      type="password"
                      show-password
                      v-model="shelfData.housePwd"
                      placeholder="请输入"
                      clearable
                      style="flex: 1"
                    />
                    <el-button
                      type="primary"
                      @click="changeSelect(shelfData, 0)"
                      >保存</el-button
                    >
                  </div>
                </el-form-item>
                ...
                <!-- 房源评级 -->
                <el-form-item label="房源评级">
                  <el-select
                    v-model="shelfData.grade"
                    placeholder="请选择"
                    style="width: 150px"
                    @change="changeSelect(shelfData, 4)"
                  >
                    <el-option
                      v-for="dict in dict.type.tfw_ecommendation_level"
                      :key="dict.value"
                ...
                <el-form-item label="房屋类型">
                  <el-select
                    v-model="shelfData.propertyType"
                    placeholder="请选择"
                    style="width: 150px"
                    @change="changeSelect(shelfData, 2)"
                  >
                    <el-option
                      v-for="dict in dict.type.tfw_property_type"
                      :key="dict.value"
                ...
                <el-form-item label="房源状态">
                  <el-select
                    v-model="shelfData.state"
                    placeholder="请选择"
                    style="width: 150px"
                    @change="changeSelect(shelfData, 1)"
                  >
                    <el-option
                      v-for="dict in dict.type.tfw_state"
                      :key="dict.value"
                      :label="dict.label"
                      :value="dict.value"
                    ></el-option>
                ...
                <el-form-item
                  label="上架状态"
                  v-if="shelfData.status == 1 || shelfData.status == 2"
                >
                  <el-switch
                    v-model="shelfData.status"
                    active-color="#13ce66"
                ...
                <!-- 验证链接 -->
                <el-form-item label="验真链接" v-if="shelfData.ifUpdate">
                  <el-link
                    :href="
                      'https://new.tianfucd.com/testing?id=' + shelfData.id
                    "
                    target="_blank"
                    type="primary"
                  >
                    {{ shelfData.ownerType == 1 ? "业主已验真" : "点击验真" }}
                  </el-link>
                </el-form-item>
                <el-form-item>
                  <div>
                    <el-select
                      v-model="tfwHouseRent.state"
                      placeholder="是否结束出租"
                      size="mini"
                      style="width: 150px; padding-right: 10px"
                    >
                      <el-option :key="0" label="出租中" :value="0"></el-option>
                      <el-option :key="1" label="未出租" :value="1"></el-option>
                    </el-select>
                    <el-button @click="saveRent(shelfData)">确定</el-button>
                  </div>
                </el-form-item>
      ...
      <el-form :model="formxq" :inline="true">
        ...
        <el-form-item label="房屋现况" prop="statusQuo" size="mini">
          <el-select v-model="formxq.statusQuo" placeholder="请选择房屋现状">
            <el-option
              v-for="dict in dict.type.twf_fwxz"
              :key="dict.value"
              :label="dict.label"
              :value="dict.value"
            >
        ...
        <el-form-item
          ...
          prop="sellingPrice"
          size="mini"
          v-if="formxq.type == 1 || formxq.type == 3"
        >
          <el-input v-model="formxq.sellingPrice" placeholder="请输入售价">
            <i slot="suffix" style="font-style: normal; margin-right: 20px"
              >万</i
            >
          </el-input>
        </el-form-item>
        <el-form-item
          ...
          prop="rental"
          size="mini"
          v-if="formxq.type == 2 || formxq.type == 3"
        >
        ...
        <el-tabs v-model="activeName" type="card">
          <el-tab-pane label="基本信息" name="first">
            <el-col :span="24">
              <el-card class="box-card">
                ...
                <el-tabs v-model="form.type" v-if="form.type">
                  <el-tab-pane
                    :name="dictitem.value"
                    v-for="dictitem in dict.type.tfw_rent_sale"
                    :key="dictitem.value"
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        <i
                          slot="suffix"
                          style="font-style: normal; margin-right: 20px"
                          >%</i
                        >
                      </el-input>
                    </el-form-item>
                    ...
                    >
                      <el-select
                        v-model="form.commissionMethod"
                        placeholder="请选承租佣金"
                      >
                        <el-option
                          v-for="dict in dict.type.tfw_house_commission_method"
                          :key="dict.value"
                          :label="dict.label"
                          :value="dict.value"
                        ></el-option>
                    ...
                    >
                      <el-select
                        v-model="form.shortestRentalPeriod"
                        placeholder="最短租期"
                      >
                        <el-option
                          v-for="dict in zqlist"
                          :key="dict.value"
                          :label="dict.label"
                          :value="dict.value"
                        ></el-option>
                    ...
                    >
                      <el-select
                        v-model="form.rentFreeTime"
                        placeholder="请选择免租时间"
                      >
                        <el-option
                          v-for="dict in mzlist"
                          :key="dict.value"
                          :label="dict.label"
                          :value="dict.value"
                        ></el-option>
            ...
            <el-col :span="24">
              <el-card class="box-card">
                ...
                <el-form-item label="原户型ID" prop="oldHuxingId" size="small">
                  <el-input
                    v-model="form.oldHuxingId"
                    placeholder="请输入原户型ID"
                    style="width: 150px"
                  />
                </el-form-item>
                <el-form-item label="户型图" prop="huxingId" size="small">
                  <el-select
                    v-model="form.huxingId"
                    placeholder="请选择户型图"
                    filterable
                    @click.native="clickhx"
                    ref="input"
                    ...
                    >
                      ...
                      <el-col :span="24">
                        <el-form-item label="面积" prop="habitableSpace">
                          <el-input
                            v-model="huxing.habitableSpace"
                            placeholder="请输入面积"
                          />
                        </el-form-item>
                        <el-form-item label="卧室数量" prop="bedRoomNum">
                          <el-input-number
                            v-model="huxing.bedRoomNum"
                            :min="0"
                            :max="100"
                          >
                          </el-input-number>
                        ...
                        <el-form-item label="备注" prop="remark">
                          <el-input
                            v-model="huxing.remark"
                            placeholder="请输入备注"
                          />
                        </el-form-item>
                      </el-col>
                      <el-col :span="12">
                        <el-button type="primary" @click="submithuxing"
                          >确 定</el-button
                        >
                ...
                <el-col :span="24">
                  ...
                  >
                    <el-checkbox-group
                      v-model="form.featuredTags"
                      :max="3"
                      v-if="form.featuredTags"
                    >
                      <el-checkbox
                        v-for="dict in dict.type.tfw_featured_tags"
                        :key="dict.value"
                        :label="dict.value"
                      >
                        {{ dict.label }}
            ...
            <div class="footerbox">
              <el-button type="primary" @click="submitaudit"
                >提交审核</el-button
              >
              <el-button type="primary" @click="savedraftsbox"
                >保存信息</el-button
              >
          ...
          <el-tab-pane label="详细信息" name="second">
            <el-card class="box-card">
              ...
              <el-form-item label="产权所有" prop="equityOwnership" size="mini">
                <el-select
                  v-model="form.equityOwnership"
                  placeholder="请选择产权所有"
                >
                  <el-option
                    v-for="dict in dict.type.tfw_property_ownership"
                    :key="dict.value"
                    :label="dict.label"
                    :value="parseInt(dict.value)"
                  ></el-option>
            ...
            <el-card class="box-card">
              ...
              <el-col :span="24">
                <el-form-item label="供暖" prop="heating" size="mini">
                  ...
                  >
                    <el-option
                      v-for="dict in dict.type.tfw_heating_type"
                      :key="dict.value"
                      :label="dict.label"
                      :value="dict.value"
                    ></el-option>
                ...
                <el-form-item label="房屋结构" prop="structure" size="mini">
                  ...
                  >
                    <el-option
                      v-for="dict in dict.type.tfw_house_structure"
                      :key="dict.value"
                      :label="dict.label"
                      :value="dict.value"
                    ></el-option>
                ...
                <el-form-item label="用电类型" prop="powerType" size="mini">
                  ...
                  >
                    <el-option
                      v-for="dict in dict.type.tfw_electricity_consumption"
                      :key="dict.value"
                      :label="dict.label"
                      :value="dict.value"
                    ></el-option>
                ...
                <el-form-item label="是否唯一" prop="ifOnly" size="mini">
                  <el-select v-model="form.ifOnly" placeholder="是否唯一">
                    <el-option
                      v-for="dict in dict.type.tfw_sfwy"
                      :key="dict.value"
                      :label="dict.label"
                      :value="dict.value"
                    >
                ...
                <el-form-item label="是否公盘" prop="ifPublic" size="mini">
                  <el-select v-model="form.ifPublic" placeholder="是否公盘">
                    <el-option
                      v-for="dict in dict.type.twf_sfgp"
                      :key="dict.value"
                      :label="dict.label"
                      :value="dict.value"
                    >
                ...
                <el-form-item label="是否贷款" prop="collateral" size="mini">
                  <el-select v-model="form.collateral" placeholder="是否贷款">
                    <el-option
                      v-for="dict in dict.type.tfw_mortgage_situation"
                      :key="dict.value"
                      :label="dict.label"
                      :value="dict.value"
                    >
                ...
                <el-form-item
                  label="交易权属"
                  prop="transactionOwnership"
                  size="mini"
                >
                  <el-select
                    v-model="form.transactionOwnership"
                    placeholder="交易权属"
                  >
                    <el-option
                      v-for="dict in dict.type.tfw_tosp"
                      :key="dict.value"
                      :label="dict.label"
                      :value="dict.value"
                    >
                ...
                <el-form-item
                  label="房源核验码"
                  prop="ifIncorporation"
                  size="mini"
                >
                  <el-input
                    v-model="form.ifIncorporation"
                    placeholder="请输入总楼层"
                    style="width: 200px"
                  >
                  </el-input>
            ...
            <el-card class="box-card">
              ...
              >
                <el-select
                  v-model="form.entrustmentMethod"
                  placeholder="请选择委托方式"
                >
                  <el-option
                    v-for="dict in dict.type.tfw_entrustment_method"
                    :key="dict.value"
                    :label="dict.label"
                    :value="dict.value"
                  ></el-option>
              ...
              <el-form-item
                label="委托来源"
                prop="entrustmentSource"
                size="mini"
              >
                <el-select
                  v-model="form.entrustmentSource"
                  placeholder="请选择委托来源"
                >
                  <el-option
                    v-for="dict in dict.type.tfw_source_commission"
            ...
            <el-card class="box-card">
              ...
              >
                <el-select
                  v-model="form.ifPlatform"
                  placeholder="是否为平台租房"
                  style="width: 150px"
                >
                  <el-option
                    v-for="dict in dict.type.tfw_ptjf"
                    :key="dict.value"
                    :label="dict.label"
                    :value="parseInt(dict.value)"
                  >
              ...
              >
                <el-input
                  v-model="form.ifRecommend"
                  placeholder="请输入是否全站推荐"
                  style="width: 140px"
                />
              </el-form-item>
            </el-card>
          ...
          <el-tab-pane label="房源照片" name="fourth" v-if="showphone">
            ...
            <div style="padding:10px 20px;">
              <div style="font-size:14px;font-weight:bold;margin-bottom:8px;">权证信息</div>
              <el-form-item label="权证业务号" size="small">
                <el-input v-model="form.certNo" placeholder="请输入权证业务号" style="width:220px" />
              </el-form-item>
              <el-form-item label="身份证号" size="small">
                <el-input v-model="form.idCard" placeholder="请输入身份证号" style="width:220px" />
              </el-form-item>
              <el-form-item label="房屋地址" size="small">
                <el-input v-model="form.houseAddr" placeholder="请输入房屋地址" style="width:400px" />
              </el-form-item>
            </div>
            <div class="footerbox">
              <el-button type="primary" @click="submitaudit"
    ...
    <el-dialog title="被转移人名单" :visible.sync="dialogTableVisible">
      <el-table size="mini" height="500px" :data="newuserhouseopenlist">
        ...
        <el-table-column fixed="right" label="操作" width="120">
          <template slot-scope="scope">
            <el-button
              @click.native.prevent="transferto(scope.row)"
              type="text"
              size="small"
            >
              转移
...
import { getTransferList } from "@/api/fangyuan/transfer";
import {
  listPhotos,
  getPhotos,
  delPhotos,
...
import lookprice from "./lookprice.vue";
...
import { editTfwHouseRent } from "@/api/fangyuan/rent";
...
export default {
  ...
  dicts: [
    ...
    "tfw_entrustment_method",
    "tfw_source_commission",
    "tfw_housing_orientation",
    "tfw_electricity_consumption",
    "tfw_featured_tags",
    "tfw_house_structure",
    "tfw_mortgage_situation",
    "tfw_new_old",
    "tfw_property_type",
    "tfw_property_ownership",
    "tfw_heating_type",
    "tfw_elevator_situation",
    "tfw_czlx",
    "tfw_mzsj",
    "tfw_zdzq",
    "tfw_czr",
  ...
  created() {
    ...
    console.log(this.$store.getters.userCities, "userCities");
    this.regioncode = [
  ...
  methods: {
    ...
    qrcodeclick() {
      if (!this.visible) {
        ...
        qrcode({ houseId: this.form.id }).then((res) => {
          this.qrcode =
            "https://new.tianfucd.com/imageupload?id=" +
            this.form.id +
            "&isType=1";
    ...
    changeSelect(val, type) {
      ...
      if (type === 0 && val.housePwd != "***") {
        dataform.housePwd = val.housePwd;
      }
      if (dataform.grade == 3) {
        let isok = true;
        window.location.href =
          "https://new.tianfucd.com/testing?isashow=" + isok + "&id=" + val.id;
      } else {
    ...
    //获取区域列表
    getTownList() {
      getprovincelist1(this.cityCode).then((response) => {
    ...
    // 详情
    handlerowlist(row) {
      this.drawerIfUpdate = row.ifUpdate;
      ...
      getFangyuan(id).then((response) => {
        ...
        if (this.form.tfwHouseRentList.length > 0) {
          this.tfwHouseRent = this.form.tfwHouseRentList[0];
          this.tfwHouseRent.date = [
            this.tfwHouseRent.startDate,
            this.tfwHouseRent.endDate,
          ];
        }
      });
    },
    ...
    handlegenjin(row) {
      ...
      getFangyuan(this.genjinid).then((response) => {
        ...
        if (this.form.kfang) this.form.kfang = "";
        this.form.phoneNumber = JSON.parse(this.form.phoneNumber);
        this.showdialog = true;
        this.showphone = true;
    ...
    initRent(data) {
      if (data.length > 0)
        return (
          data[0].startDate.split("T")[0] + "至" + data[0].endDate.split("T")[0]
        );
      else return "未设置";
    },
    saveRent(shelfData) {
      if (this.tfwHouseRent.date != null) {
        this.tfwHouseRent.startDate = this.tfwHouseRent.date[0];
        this.tfwHouseRent.endDate = this.tfwHouseRent.date[1];
        this.tfwHouseRent.houseId = shelfData.id;
        editTfwHouseRent(this.tfwHouseRent).then((res) => {
          console.log(res, "res");
        });
      }
    },
