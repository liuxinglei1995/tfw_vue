  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" label-width="60px" label-position="right">
      ...
      <el-row>
        ...
        <el-form-item label="区域选择" prop="areaCode" size="mini" label-width="80px">
          <el-select v-model="queryParams.areaCode" clearable placeholder="请选择" style="width:120px">
            <el-option v-for="c in townList" :key="c.id" :value="c.id" :label="c.extName"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="房源状态" prop="state" size="mini" label-width="80px">
          <el-form-item label="房源状态" prop="state" size="mini" label-width="80px">
            <el-select v-model="queryParams.state" placeholder="请选择房源状态" style="width:130px">
              <el-option v-for="  dict   in   tfw_house_state  " :key="dict.value" :label="dict.label"
                         :value="dict.value"></el-option>
            </el-select>
          </el-form-item>
        </el-form-item>
      ...
      <div class="container" v-show="showSearch">
        <div class="CationBox">
          <el-form-item label="房源评级" prop="grade" size="mini" label-width="80px">
            <el-select v-model="queryParams.grade" placeholder="请选择星级" style="width:120px">
              <el-option v-for=" dict in dict.type.tfw_ecommendation_level" :key="dict.value" :label="dict.label"
                         :value="dict.value"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="房屋来源" prop="entrustmentMethod" size="mini" label-width="80px">
            <el-select v-model="queryParams.entrustmentMethod" placeholder="请选择情况" style="width:120px">
              <el-option v-for=" dict in dict.type.tfw_source_commission" :key="dict.value" :label="dict.label"
                         :value="dict.value"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="维护人" prop="createByName" size="mini">
            <el-input v-model="queryParams.createByName" placeholder="请输入维护人" clearable style="width:120px" />
          </el-form-item>
          <el-form-item label="所属门店" prop="deptId" label-width="80px">
            <treeselect v-model="queryParams.deptId" append-to-body :options="deptOptions" :appendToBody="true"
                        :show-count="true" placeholder="请选择归属部门" style="width:300px" />
          </el-form-item>
          <el-form-item label="产权年限" prop="ownershipYears" size="mini" label-width="80px">
            <el-select v-model="queryParams.ownershipYears" placeholder="请选择产权年限" style="width:130px">
              <el-option v-for="  dict   in   dict.type.tfw_property_rights  " :key="dict.value" :label="dict.label"
                         :value="dict.value"></el-option>
            </el-select>
          </el-form-item>

          <el-form-item label="有钥匙房" prop="kfang" size="mini" label-width="80px">
            <el-select v-model="queryParams.kfang" placeholder="请选择情况" style="width:120px">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList">更多搜索</right-toolbar>
    </el-row>
    <!-- 显示列表 -->
    <el-table size="mini" v-loading="loading" :data="fangyuanList" @selection-change="handleSelectionChange"
          ...
          <div style="display: flex;align-items: center">
            ...
            <div
              ...
              <div style="display: flex;flex-wrap: wrap">
                ...
                <div class="jbxx" :class="[scope.row.status == 2 ? 'four' : 'five']">
                  <dict-tag :options="dict.type.tfw_status" :value="scope.row.status" type="danger"/>
                </div>
                <div class="jbxx">
      ...
      <el-table-column label="总价" header-align="center" align="center" prop="sellingPrice,propertyArea" width="180px">
        <template slot-scope="scope">
          ...
          <div style="color:#FF7200;margin-bottom:5px" v-if="scope.row.state === '4' || scope.row.state === '6'">
      ...
      <el-form ref="form" :model="form" label-width="80px" :inline="true" :rules="rules">
        ...
        <el-col :span="24">
          ...
          <el-form-item v-for="(   domain, index   ) in    phoneNumbercopy   " :label="'业主' + (index + 1)"
                        :key="domain.key" :prop="'phoneNumbercopy.' + index + '.value'">
        ...
        <el-tabs v-model="activeName2" type="card">
          <el-tab-pane label="基本信息" name="first">
            <div class="messagebox">
              ...
              <div class="rightmessagebox">
                ...
                <p class="adress">地址:{{ form.building ? form.building : "-" }}栋{{ form.unit ? form.unit : "-" }}单元{{
                    form.floor ?
                      form.floor : "-" }}层{{ form.roomNum ? form.roomNum : "-" }}号
                  <img src="@/assets/image/closeeyes.png" alt="" style="width: 20px;height: 20px;">
                  <img src="@/assets/image/openeyes.png" alt="" style="width: 20px;height: 20px;color: blue;">
                </p>
            ...
            <el-descriptions title="业主信息" border :column="3" style="padding-right:20px;">
              ...
              <el-descriptions-item label="房号">{{ form.building ? form.building : "-" }}栋{{ form.unit ? form.unit :
                "-" }}单元{{ form.floor ? form.floor : "-" }}层{{ form.roomNum ? form.roomNum : "-"
          ...
          <el-tab-pane label="上架详情" name="shelf">
            ...
            <el-form label-position="left" class="demo-table-expand">-->
              ...
              <el-form-item label="租期" v-if="shelfData.state === '4' || shelfData.state === '6'">
                <div style="display: flex">
                  <el-date-picker
                    v-model="tfwHouseRent.date"
                    type="datetimerange"
              ...
              <el-form-item>
                <div>
                  <el-select v-model="tfwHouseRent.state" placeholder="是否结束出租" size="mini"
                             style="width:150px;padding-right:10px ;">
                    <el-option :key="0" label="出租中"
                               :value="0"></el-option>
                    <el-option :key="1" label="未出租"
                               :value="1"></el-option>
                  </el-select>
...
export default {
  ...
  methods: {
    ...
    handlerowlist(row) {
      ...
      getFangyuan(id).then(response => {
        ...
        if(this.form.tfwHouseRentList.length > 0) {
          ...
          this.tfwHouseRent.date = [this.tfwHouseRent.startDate,this.tfwHouseRent.endDate];
        }
      })
    },
    //修改详情
    ...
    initRent(data) {
      if(data.length > 0) return data[0].startDate.split('T')[0]  + '至' + data[0].endDate.split('T')[0]
      else return '未设置'
    },
    saveRent(shelfData) {
