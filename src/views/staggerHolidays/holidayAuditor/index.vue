...
<script>
import { auditorListHolidays, updateHolidays } from "@/api/staggerHolidays/staggerHolidays";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      auditorListHolidays(this.queryParams).then(response => {
        ...
        for (const holidays of response.rows) {
          ...
          console.log(holidays.dateList.length > 0)
        }
