<template>
    <div class="app-container">
        <el-table v-loading="loading" :data="leaveList" @selection-change="handleSelectionChange">
            ...
            <el-table-column label="带看类型" align="center" prop="type">
                <template slot-scope="scope">
                    <dict-tag :options="dict.type.tfw_look_type" :value="scope.row.type ? scope.row.type : ''" />
                </template>
            </el-table-column>
            <el-table-column label="带看人员" align="center" prop="createName">
            </el-table-column>
            <el-table-column label="陪看人" align="center" prop="andUser">
                <template slot-scope="scope">
                    <div v-if="scope.row.andUser" class="anduser">
                        <div v-for="item in scope.row.andUser">
            ...
            <el-table-column label="带看时间" align="center" prop="customerReportingTime" width="140">
                <template slot-scope="scope">
                    <span style="font-size:12px ;">{{ scope.row.customerReportingTime }}</span>
                </template>
            </el-table-column>
            <el-table-column label="结束时间" align="center" prop="leaveEndTime" width="140">
                <template slot-scope="scope">
                    <span style="font-size:12px ;">{{ parseTime(scope.row.leaveEndTime, '{y}-{m}-{d}') }}</span>
                </template>
            </el-table-column>
            ...
            <el-table-column label="操作" align="center" class-name="small-padding fixed-width" >
                <template slot-scope="scope">
                    ...
                    <el-button v-if="0 == scope.row.state" size="mini" type="text" icon="el-icon-edit"
                        @click="checkTheSchedule(scope.row)" v-hasPermi="['system:reporting:edit']">查看进度
                    </el-button>
                </template>
            </el-table-column>
        </el-table>
        ...
        <el-dialog :visible.sync="modelVisible" title="进度查询" width="1400px" append-to-body>
            <div style="position:relative;height: 100%;">
                <iframe id="iframe" :src="modelerUrl" frameborder="0" width="100%" height="620px" scrolling="auto"></iframe>
            </div>
        </el-dialog>
        ...
        <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
            <el-form ref="form" :model="form" :rules="rules" label-width="80px">
                ...
                <el-form-item label="开始时间" prop="leaveStartTime">
                    <el-date-picker clearable size="small" style="width: 200px" v-model="form.leaveStartTime" type="date"
                        value-format="yyyy-MM-dd" placeholder="选择开始时间">
                    </el-date-picker>
                </el-form-item>
                <el-form-item label="结束时间" prop="leaveEndTime">
                    <el-date-picker clearable size="small" style="width: 200px" v-model="form.leaveEndTime" type="date"
...
export default {
    ...
    created() {
        ...
        if (this.type == 1) {
            ...
            this.queryParams.houseId = this.customerId
        } else if (this.type == 3) {
            this.queryParams.xqId = this.customerId
        }
        this.getList()
